...
function valid_registration ($password, $email)
{
	...
	if (mysqli_num_rows($email_query) || !preg_match('#[a-zA-Z0-9_]+@princeton\.edu#i', $email) || strlen($password) < 7 || strlen($password) > 100)
	{
		return false;
       }
	else
...
function display_questions ()
{
	...
	while ($title = mysqli_fetch_array($Q_query, MYSQLI_NUM))
	{
		$title = safe($title[0], "html");
...
function display_responses ($debate)
{
	...
	while ($content = mysqli_fetch_array($ans_query, MYSQLI_NUM))
	{
		...
		//$prnt = $content[5] == 1 ? "<span class=\"agree\">\n$contents\n\nBy $username on $date\n<hr />\n</span>" : "<span class=\"disagree\">\n$contents\n\nBy $username on $date\n<hr />\n</span>";
