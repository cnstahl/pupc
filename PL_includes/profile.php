...
if (administrator() && !$_GET["profile"])
{
	...
	if ($debate_submit == "Submit")
	{
		...
		$ask_result = "Profile saved successfully!";
	}
	else
	{
		$ask_result = "Your profile content is too long!";
	}
?>
<h1 id="greeting">Profile</h1>
<br />
<form method="post" id="greeting_block">  
	<h3 class="greeting_step">debate title --> </h3><input type="text" name="title" class="greeting_desc"></input><br />
	<h3 class="greeting_step">debate description --> </h3><textarea name="description" class="greeting_desc"></textarea><br />
	<h3 class="greeting_step">debate video-id --> </h3><input type="text" name="video_id" class="greeting_desc" /><br />
	<h3 class="greeting_step">debate bio --> </h3><textarea name="bio" class="greeting_desc"></textarea><br />
	<h3 class="greeting_step">submit --> </h3><input type="submit" name="debate_submit" value="Submit" class="home_buttons"></input>
</form>
<?php
	if ($submit)
	{
	...
	echo "<br />Profile info list:<br />";
	display_debates();
}
elseif (!administrator() && !$_GET["profile"])
{
	//echo "<br />debates:<br />";
	display_debates();
}
